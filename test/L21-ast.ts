...
export type CompoundExp = AppExp | IfExp | ProcExp | ForExp;    //added FOR
...
//********FOR***************
export const makeForExp = (loopv: VarDecl, startv: NumExp, endv: NumExp, Body: CExp) : ForExp => 
    ({tag: "ForExp", var: loopv, start: startv, end: endv, body: Body});
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isForExp(x);   //ADDED FOR
...
import { parse as parseSexp, isToken, isSexp } from "../imp/parser";
...
const parseForExp = (params : Sexp[]): Result<ForExp> => 
    ...
    praseGoodForExp(params);


const praseGoodForExp = (params : Sexp[]): Result<ForExp> =>{
    const dec=parseL21Exp(params[0]);
